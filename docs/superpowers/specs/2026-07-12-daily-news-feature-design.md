...
- **资讯不进入市场图谱**：虽然资讯与报告共享 `entities` 实体标签体系，但资讯节点不会出现在 Obsidian 市场图谱的可视化中。图谱仍然只展示报告与实体的关联网络。共享实体标签的目的是为了在资讯详情页底部推荐相关报告，以及未来可能的跨内容搜索。
...
### 4.1 主页资讯区域
- 位于 Hero 区域下方。
- 根据用户的喜好展示 3~6 条资讯卡片（标题 + 摘要 + 区域/行业标签 + 发布时间）。用户喜好来源于 `localStorage` 中保存的筛选偏好（区域/国家/行业），若无偏好记录则展示最新资讯。
- 点击卡片跳转到资讯详情页。
