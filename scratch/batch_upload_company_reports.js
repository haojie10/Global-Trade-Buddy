...
const reportDir = 'D:\\我的APP\\客户档案\\公司报告';
...
function uploadOne(filePath) {
  return new Promise((resolve) => {
    ...
    const products = extractMeta(html, 'products') || extractMeta(html, 'product') || '家居用品, 日用百货, 食品';
      ...
      res.on('end', () => {
        try {
          ...
          if (res.statusCode === 200) {
            console.log(`[OK] (${companyName || title}) -> ID: ${json.id}`);
            resolve({ file: fileName, success: true, id: json.id, title, category });
          } else {
            console.error(`[ERR] (${fileName}) Status ${res.statusCode}: ${data.substring(0, 150)}`);
...
async function run() {
  ...
  console.log(`Found ${files.length} company reports to upload in ${reportDir}:\n`);
  ...
  console.log('\n========================================');
  console.log('       COMPANY REPORT UPLOAD SUMMARY    ');
  console.log('========================================');
