...
const reportDir = 'D:\\我的APP\\客户档案\\北美\\美国';
...
function uploadOne(filePath) {
  return new Promise((resolve) => {
    ...
    const regions = extractMeta(html, 'regions') || extractMeta(html, 'region') || '美国, 北美';
    const products = extractMeta(html, 'products') || extractMeta(html, 'product') || '家居用品, 日用百货, 五金, 工具';
...
async function run() {
  ...
  console.log(`Found ${files.length} US reports to upload in ${reportDir}:\n`);
  ...
  console.log('\n========================================');
  console.log('         US REPORT UPLOAD SUMMARY       ');
  console.log('========================================');
