...
const reportDir = 'D:\\我的APP\\GTB资源\\Report\\德国';
...
function uploadOne(filePath) {
  return new Promise((resolve) => {
    ...
    if (!category) {
      if (fileName.includes('-insight') || fileName.includes('-retail-insight')) {
        category = 'product';
      } else {
        category = 'customer';
      }
    ...
    const regions = extractMeta(html, 'regions') || extractMeta(html, 'region') || '德国, 欧洲';
    const products = extractMeta(html, 'products') || extractMeta(html, 'product') || '照明产品, 五金, 工具';
...
async function run() {
  ...
  console.log(`Found ${files.length} German reports to upload in ${reportDir}\n`);
  ...
  console.log('\n========================================');
  console.log('       GERMAN UPLOAD SUMMARY            ');
  console.log('========================================');
