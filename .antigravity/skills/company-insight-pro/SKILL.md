...
1. 运行查重脚本（或调用 API）：
   ```bash
   python bin/check_report.py --company "目标公司/品牌名 (如: Action, Screwfix)"
   ```
2. **判断逻辑**：
   - **若已存在相关报告**：立即向用户汇报已有报告的标题、发布时间与查看链接，并询问：
     > *“平台报告大厅中已收录《[报告标题]》（查看链接：[url]）。请问您是直接查阅现有报告，还是需要基于最新数据重新生成并覆盖更新？”*
     若用户指示仅需查阅，则直接结束，无需重复消耗 Token；若用户明确要求重新生成，则继续后续步骤。
   - **若不存在该报告**：直接进入下一步开始深度调研。
...
- **交互式报告生成**：使用 [报告模板 HTML](assets/report-template.html) 作为基础，并完整配置变量占位符 `{{CHART1_OPTION}}`、`{{CHART2_OPTION}}` 与 `{{CHART3_OPTION}}`。
