...
const reportDir = 'D:\\我的APP\\GTB资源\\Report';
...
function uploadOne(filePath) {
  return new Promise((resolve) => {
    ...
    const regions = extractMeta(html, 'regions') || extractMeta(html, 'region') || '欧洲';
    const products = extractMeta(html, 'products') || extractMeta(html, 'product') || '照明产品, 家居用品, 五金';
...
async function run() {
  // 只读取 D:\我的APP\GTB资源\Report 直接目录下的 HTML 文件，不进入子目录
  const files = fs.readdirSync(reportDir)
    .filter(f => f.endsWith('.html') && fs.statSync(path.join(reportDir, f)).isFile());
    
  console.log(`Found ${files.length} direct HTML reports in ${reportDir} (excluding subdirectories):\n`);
  ...
  console.log('\n========================================');
  console.log('       DIRECT REPORT UPLOAD SUMMARY     ');
  console.log('========================================');
