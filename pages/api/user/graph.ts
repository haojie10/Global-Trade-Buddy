...
export async function getGraphData(userId: string, userRole: string, dbClient: any) {
  ...
  // 3. 预先构建"实体 -> 报告列表"的倒排索引，只对真正共享实体的报告对跑判定矩阵
  // 时间复杂度从 O(N²) 降为 O(E × k)，其中 E 为实体数、k 为平均每个实体关联的报告数
  const entityToReports = new Map<string, string[]>();
  for (const [reportId, entsMap] of reportEntitiesMap.entries()) {
    for (const entId of entsMap.keys()) {
      if (!entityToReports.has(entId)) {
        entityToReports.set(entId, []);
      }
      entityToReports.get(entId)!.push(reportId);
    }
  }

  // 收集所有候选报告对（共享至少一个实体，或互为对方主体的提及方）
  const candidatePairs = new Set<string>();
  for (const reportIds of entityToReports.values()) {
  ...
  // 主实体提及也可能构成边（即使双方没有共同实体，一方报告里提到了另一方主体）
  for (const node of reportNodes) {
    if (!node.primary_entity_id) continue;
    const mentioning = entityToReports.get(node.primary_entity_id) || [];
    for (const otherReportId of mentioning) {
      if (otherReportId === node.id) continue;
      const a = node.id;
      const b = otherReportId;
      candidatePairs.add(a < b ? `${a}|${b}` : `${b}|${a}`);
    }
  }
  ...
  for (const node of reportNodes) {
    reportById.set(node.id, node);
  }

  // 仅对候选报告对执行关系判定矩阵
  for (const pairKey of candidatePairs) {
    const [idA, idB] = pairKey.split('|');
    const rA = reportById.get(idA);
    const rB = reportById.get(idB);
    if (!rA || !rB) continue;
