...
const reportDir = 'D:\\我的APP\\客户档案\\大洋洲\\澳大利亚';
...
function uploadOne(filePath) {
  return new Promise((resolve) => {
    ...
    const regions = extractMeta(html, 'regions') || extractMeta(html, 'region') || '澳大利亚, 大洋洲';
    const products = extractMeta(html, 'products') || extractMeta(html, 'product') || '家居用品, 男女装, 玩具, 日用百货';
...
async function run() {
  ...
  console.log(`Found ${files.length} Australian reports to upload in ${reportDir}:\n`);
  ...
  console.log('\n========================================');
  console.log('       AUSTRALIA UPLOAD SUMMARY         ');
  console.log('========================================');
