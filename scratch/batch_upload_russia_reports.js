...
const reportDir = 'D:\\我的APP\\GTB资源\\Report\\俄罗斯';
...
function uploadOne(filePath) {
  return new Promise((resolve) => {
    ...
    const regions = extractMeta(html, 'regions') || extractMeta(html, 'region') || '俄罗斯, 欧洲';
    const products = extractMeta(html, 'products') || extractMeta(html, 'product') || '家居用品, 玩具, 办公文具, 照明产品, 食品';
...
async function run() {
  ...
  console.log(`Found ${files.length} Russian reports to upload in ${reportDir}:\n`);
  ...
  console.log('\n========================================');
  console.log('       RUSSIA REPORT UPLOAD SUMMARY     ');
  console.log('========================================');
