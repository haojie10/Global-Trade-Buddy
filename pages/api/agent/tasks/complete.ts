import { NextApiRequest, NextApiResponse } from 'next';
import { PoolClient } from 'pg';
import { withDb } from '../../../../lib/api-handler';

function validateAgentAuth(req: NextApiRequest): boolean {
  const authHeader = req.headers.authorization;
  const token = (authHeader && authHeader.split(' ')[1]) || (req.headers['x-agent-key'] as string);
  const isProd = process.env.NODE_ENV === 'production';
  const expectedToken = process.env.AGENT_API_KEY;

  if (isProd && !expectedToken) return false;
  return isProd
    ? (Boolean(token) && token === expectedToken)
    : (token === (expectedToken || 'automation_agent_secret') || token === 'automation_agent_secret');
}

async function completeHandler(req: NextApiRequest, res: NextApiResponse, dbClient: PoolClient) {
  if (!validateAgentAuth(req)) {
    return res.status(401).json({ error: 'Unauthorized: Invalid Agent API Key' });
  }

  const { task_id, report_id, report_url } = req.body || {};

  if (!task_id || !report_id) {
    return res.status(400).json({ error: 'Missing required parameters: task_id or report_id' });
  }

  try {
    const finalReportUrl = report_url || `https://marketgraphic.cn/reports/${report_id}`;

    const updateRes = await dbClient.query(
      `UPDATE research_tasks
       SET status = 'completed',
           report_id = $1,
           report_url = $2,
           updated_at = NOW()
       WHERE id = $3
       RETURNING id, company_name, seq_no, status`,
      [report_id, finalReportUrl, task_id]
    );

    if (updateRes.rows.length === 0) {
      return res.status(404).json({ error: 'Task not found' });
    }

    return res.status(200).json({
      success: true,
      message: '任务已成功标记完成并挂接报告',
      task: updateRes.rows[0]
    });
  } catch (err: any) {
    console.error('Error completing research task:', err);
    return res.status(500).json({ error: 'Failed to complete task', details: err.message });
  }
}

export default withDb(completeHandler, { methods: ['POST'] });
